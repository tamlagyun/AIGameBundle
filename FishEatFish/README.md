...
《鲫鱼吃鲤鱼》是面向多平台发布的横屏 2D 海洋多人动作游戏。当前版本已支持海洋地图、玩家移动、同房间同步、服务器权威战斗、技能配置和角色变身。
...
下一阶段建议实现不依赖 Cocos 的最小玩法闭环：移动、普通撕咬、鲫鱼抢食、受伤、击败、经验和升级回血，再连接 Cocos 场景表现。
...
打开 `MainScene` 后运行即可查看海底地图、动画角色、左下虚拟摇杆、右下技能区，以及右上“技能配置/变身”入口。变身界面当前可在小鲫鱼和大王乌贼之间切换，并同步同房间玩家。形态配置说明见 `docs/appearance-configuration.md`；提示词和生成记录位于 `docs/art-prompts.md`。
